...
public static void main(String[]args) {
    ...
    String className = hashMap.get(new Student("lilei","class1"));
...
为什么呢？因为我们并没有重写`hashcode`，所以即使我们去`get`的时候传入的`Student`的name以及classname与`put`的时候的对象值是一样的，它们的`hashcode`也是不一样的，所以当我们期望`get`到的是`class1`的时候，我们需要重写`hashCode`方法！   


好吧,老实说，我并没有做到!因为一般来说我不会把`Student`这样的类当做一个`Key`去处理  
