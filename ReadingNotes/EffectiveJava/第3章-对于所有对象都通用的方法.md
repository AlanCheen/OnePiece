...
为什么呢？因为我们并没有重写`hashcode`，所以即使我们去`get`的时候传入的`Student`的name以及classname与`put`的时候的对象值是一样的，也即**它们是`equals`的**，但是要注意，**它们的`hashcode`是不一样的**，这样就违反了上面所说的**equals相等，hashCode也要相等**的原则，所以当我们期望`get`到的是`class1`的时候，我们需要重写`hashCode`方法，让它们的hashcode相同！   
